...
import com.mrbonono63.create.content.logistics.InWorldProcessing.Type;
...
public class AirCurrent {
	...
	protected List<Pair<TransportedItemStackHandlerBehaviour, Type>> affectedItemHandlers =
		new ArrayList<>();
	...
	protected void tickAffectedEntities(World world, Direction facing) {
		for (Iterator<Entity> iterator = caughtEntities.iterator(); iterator.hasNext();) {
			...
			entityDistance -= .5f;
			Type processingType = getSegmentAt((float) entityDistance);
			if (entity instanceof ServerPlayerEntity)
	...
	public void rebuild() {
		...
		for (int i = searchStart; i * searchStep <= searchEnd * searchStep; i += searchStep) {
			...
			Type newType = Type.byBlock(world, currentPos);
	...
	public void tickAffectedHandlers() {
		for (Pair<TransportedItemStackHandlerBehaviour, Type> pair : affectedItemHandlers) {
			...
			Type processingType = pair.getRight();
	...
	public Type getSegmentAt(float offset) {
